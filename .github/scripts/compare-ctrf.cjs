...
md += `**Legend:** ✅ Pass | ❌ Fail | ✅🔧 Fixed | ❌📉 Regressed | ✅🆕 New (pass) | ❌🆕 New (fail) | 🗑️ Removed | <sub>str</sub>=streaming <sub>blk</sub>=blocking <sub>a</sub>=async <sub>s</sub>=sync <sub>io</sub>=stdio <sub>sse</sub>=sse <sub>hi</sub>=highlevel <sub>lo</sub>=lowlevel
