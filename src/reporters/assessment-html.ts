...
function targetDetail(target: TargetAssessment): string {
	return `<div class="target-detail"><div class="variant-header"><span>variant</span><span class="variant-meta">configuration / versions</span><span class="variant-findings">findings</span><span class="variant-score">score</span><span class="variant-trend">trend</span></div><div class="variant-stack">${target.variants
		.map((variant, index) => {
			...
			const linkLabel = `Link to variant ${index + 1}`;
			return `<details id="${escapeHtml(fragment)}" class="variant-panel"><summary><strong class="variant-name">variant ${index + 1}<a class="variant-permalink" href="#${escapeHtml(fragment)}" data-variant-link aria-label="${linkLabel}" title="${linkLabel}"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M6.5 9.5l3-3M5 11H4a3 3 0 0 1 0-6h3M9 5h3a3 3 0 0 1 0 6H9" fill="none" stroke="currentColor" stroke-linecap="round"/></svg></a></strong><span class="variant-meta">${variantMetadata(variant)}</span><strong class="variant-findings">${findingsHover(variant.findings)}</strong><strong class="variant-score rating-${classifyScore(effectiveScore(variant), variant.completion)}">${effectiveScore(variant)}</strong><span class="variant-trend">${scoreSparkline("variant", variant.id)}</span></summary>${variantDetail(variant)}</details>`;
		})
		.join("")}</div></div>`;
}
...
document.addEventListener('click',event=>{const variantLink=event.target.closest('[data-variant-link]');if(variantLink)event.stopPropagation();const spanLink=event.target.closest('[data-span-target]');if(spanLink)setTimeout(()=>revealSpan(spanLink.dataset.spanTarget),0);});
