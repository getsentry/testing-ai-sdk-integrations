...
import {
	executionDetails,
	executionSummary,
} from "./assessment-execution-html.js";
...
function scoreExplanation(
	...
	const calculation =
		scoringVersion === "4"
			? `<p>Telemetry quality is scored across observed domains, independently of execution coverage. No usable evidence means <strong>not assessed</strong>, not a passing score. Unknown scores are excluded from averages; coverage stays visible.</p><p>Only calls actually attempted are evaluated. Failed calls do not require successful-response fields. Interrupted telemetry delivery cannot prove an absent span or attribute was never emitted.</p><p>Tool arguments and results are compared with independent execution records. Findings from every usable attempt are retained. Incomplete runs are excluded from comparative trends, but remain in the report history.</p>`
			: scoringVersion === "3"
	...
	return `<details class="score-explanation"><summary><span class="score-explanation-caret" aria-hidden="true">›</span>How scoring works</summary><div class="score-explanation-body"><div class="score-copy">${calculation}</div><div class="score-bands" aria-label="Score color ranges"><span class="score-band score-band-good"><strong>85-100</strong> good</span><span class="score-band score-band-warning"><strong>70-84</strong> improvements needed</span><span class="score-band score-band-bad"><strong>0-69</strong> significant improvements needed</span><span class="score-band score-band-bad"><strong>incomplete</strong> out of spec</span></div></div></details>`;
}
	...
	return finding.occurrences
		.map((occurrence) => {
			const attempt = occurrence.attemptId
				? ` · attempt ${escapeHtml(occurrence.attemptId.split("/").at(-1) ?? occurrence.attemptId)}`
				: "";
			if (occurrence.evidence.length === 0) {
				return `<p class="finding-evidence-empty"><code>${escapeHtml(occurrence.probeId)}</code>${attempt} has no captured span evidence</p>`;
			}
			return `<div class="finding-evidence"><span>assessment call <code>${escapeHtml(occurrence.probeId)}</code>${attempt}</span><ul>${occurrence.evidence
				.map((evidence) => {
...
function compactTargetAssessment(target: TargetAssessment): string {
	...
	return `<div class="compact-assessment rating-${rating}"><strong>${target.telemetryScore === null ? "—" : score}</strong></div>`;
}
...
function variantDetail(
	...
	const runtimeFailures = assessment.runtimeFailures.length
		? `<section class="detail-section runtime-section"><div class="section-heading"><h4>runtime information</h4><span>${assessment.runtimeFailures.length} event${assessment.runtimeFailures.length === 1 ? "" : "s"}</span></div><ul class="failures">${assessment.runtimeFailures
				.map(
					(failure) =>
						`<li class="${failure.recovered || failure.secondary ? "non-stopping" : "stopping"}"><code>${escapeHtml(failure.category ?? failure.kind)} / ${escapeHtml(failure.kind)}</code><span>${escapeHtml(failure.message)}${failure.probeId ? ` · ${escapeHtml(failure.probeId)}` : ""}</span><strong>${failure.secondary ? "secondary symptom" : failure.recovered ? "recovered on retry" : "unrecovered"}</strong></li>`,
				)
				.join("")}</ul></section>`
		: "";
	return `<div class="detail-content">
		${executionDetails(assessment)}${runtimeFailures}${findingDetails(assessment)}${spanDetails(assessment)}
			...
			(group) =>
				`<div class="variant-comparison">${group
					.map((variant) => {
						...
						return `<details id="${escapeHtml(fragment)}" class="variant-panel"><summary><strong class="variant-name">${escapeHtml(versionLabel)}<a class="variant-permalink" href="#${escapeHtml(fragment)}" data-variant-link aria-label="${escapeHtml(linkLabel)}" title="${escapeHtml(linkLabel)}"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M6.5 9.5l3-3M5 11H4a3 3 0 0 1 0-6h3M9 5h3a3 3 0 0 1 0 6H9" fill="none" stroke="currentColor" stroke-linecap="round"/></svg></a></strong><span class="variant-meta">${variantMetadata(variant)}</span><strong class="variant-findings">${findingsHover(variant.findings)}</strong><strong class="variant-score rating-${classifyScore(effectiveScore(variant), variant.completion)}">${variant.telemetryScore === null ? "—" : effectiveScore(variant)}</strong><span class="variant-trend">${scoreSparkline("variant", variant.id)}</span></summary>${variantDetail(variant, programSources.get(variant.id))}</details>`;
					})
					.join("")}</div>`,
		)
		.join("");
...
export function renderAssessmentHtml(
	...
	const currentTrendEntry = {
		...
		integrations: report.targets.map((target) => ({
			...
			score:
				target.telemetryScore === null ? null : effectiveTargetScore(target),
			variants: target.variants.map((variant) => ({
				id: variant.id,
				completion: variant.completion,
				score: variant.telemetryScore === null ? null : effectiveScore(variant),
			})),
		})),
	};
...
<details><summary>Retained report attempts</summary><ul id="report-history"></ul></details>
...
const search=document.getElementById('search');const summaryScore=document.getElementById('summary-score');const summaryVariants=document.getElementById('summary-variants');const summaryFindings=document.getElementById('summary-findings');const trendTooltip=document.getElementById('trend-tooltip');let searchActive=false;let trendEntries=[];
...
function showTrendTooltip(dot){const svg=dot.ownerSVGElement;const svgRect=svg.getBoundingClientRect();const panelRect=trendTooltip.parentElement.getBoundingClientRect();const viewBox=svg.viewBox.baseVal;trendTooltip.textContent=dot.dataset.tooltip;trendTooltip.hidden=false;const pointLeft=svgRect.left-panelRect.left+(Number(dot.getAttribute('cx'))-viewBox.x)*svgRect.width/viewBox.width;const halfWidth=trendTooltip.offsetWidth/2;trendTooltip.style.left=Math.min(Math.max(pointLeft,halfWidth+4),panelRect.width-halfWidth-4)+'px';trendTooltip.style.top=svgRect.top-panelRect.top+(Number(dot.getAttribute('cy'))-viewBox.y)*svgRect.height/viewBox.height+'px';}
function hideTrendTooltip(){trendTooltip.hidden=true;}
function renderOverallTrend(entries){const svg=document.getElementById('overall-trend');hideTrendTooltip();svg.replaceChildren();document.getElementById('trend-runs').textContent=entries.length+' comparable attempt'+(entries.length===1?'':'s');if(!entries.length)return;const left=42,right=975,top=12,bottom=148;const x=index=>entries.length===1?(left+right)/2:left+index*(right-left)/(entries.length-1);const y=score=>bottom-Number(score)*(bottom-top)/100;for(const score of[0,25,50,75,100]){svg.appendChild(svgElement('line',{x1:left,y1:y(score),x2:right,y2:y(score),class:'trend-grid'}));const label=svgElement('text',{x:left-8,y:y(score)+4,'text-anchor':'end',class:'trend-label'});label.textContent=score;svg.appendChild(label);}const values=entries.map(entry=>entry.score);const color=scoreColor(values.at(-1));svg.appendChild(svgElement('polyline',{points:values.map((value,index)=>x(index)+','+y(value)).join(' '),class:'trend-line',stroke:color}));const labelStep=Math.max(1,Math.ceil(entries.length/8));entries.forEach((entry,index)=>{const tooltip=(entry.generatedAt||entry.date)+' · score '+entry.score;const dot=svgElement('circle',{cx:x(index),cy:y(entry.score),r:5,class:'trend-dot',fill:scoreColor(entry.score),tabindex:0,'aria-label':tooltip,'data-tooltip':tooltip});dot.addEventListener('mouseenter',()=>showTrendTooltip(dot));dot.addEventListener('mouseleave',hideTrendTooltip);dot.addEventListener('focus',()=>showTrendTooltip(dot));dot.addEventListener('blur',hideTrendTooltip);svg.appendChild(dot);if(index%labelStep===0||index===entries.length-1){const label=svgElement('text',{x:x(index),y:bottom+22,'text-anchor':'middle',class:'trend-label'});label.textContent=new Date(entry.date+'T00:00:00Z').toLocaleDateString('en-US',{month:'short',day:'numeric',timeZone:'UTC'});svg.appendChild(label);}});}
...
function renderTrends(historyEntries){renderReportHistory(historyEntries);const key=entry=>entry.id||entry.generatedAt||entry.date;const byAttempt=new Map(historyEntries.map(entry=>[key(entry),entry]));byAttempt.set(key(currentTrendEntry),currentTrendEntry);trendEntries=[...byAttempt.values()].filter(entry=>entry.scoringVersion===currentTrendEntry.scoringVersion&&entry.comparisonEligible!==false&&Number.isFinite(entry.score)).sort((left,right)=>(left.generatedAt||left.date).localeCompare(right.generatedAt||right.date));renderDashboardTrend();for(const svg of document.querySelectorAll('.score-sparkline')){const kind=svg.dataset.trendFramework?'framework':svg.dataset.trendTarget?'target':'variant';const id=svg.dataset.trendFramework||svg.dataset.trendTarget||svg.dataset.trendVariant;renderSparkline(svg,trendValues(trendEntries,kind,id));}}
function historyUrl(){return new URL(location.pathname.includes('/reports/')?location.pathname.split('/reports/')[0]+'/history.json':'history.json',location.href);}
function renderReportHistory(entries){const list=document.getElementById('report-history');list.replaceChildren();for(const entry of [...entries].reverse()){const reportPath=entry.reportPath||'reports/'+entry.date;if(!reportPath.startsWith('reports/')||!reportPath.split('/').every(part=>/^[a-zA-Z0-9_-]+$/.test(part)))continue;const item=document.createElement('li');const link=document.createElement('a');link.href=new URL(reportPath+'/index.html',historyUrl()).href;link.textContent=(entry.generatedAt||entry.date)+' · '+(entry.comparisonEligible===false?'incomplete / not comparable':'report')+(entry.runAttempt?' · attempt '+entry.runAttempt:'');item.appendChild(link);list.appendChild(item);}}
function loadTrends(){renderTrends([currentTrendEntry]);fetch(historyUrl(),{cache:'no-store'}).then(response=>{if(!response.ok)throw new Error('history unavailable');return response.json();}).then(history=>{if((history.schemaVersion==='3'||history.schemaVersion==='4')&&Array.isArray(history.entries))renderTrends(history.entries);}).catch(()=>{});}
function updateSummary(matches){const incomplete=rowTotal(matches,'incomplete');const scored=matches.filter(row=>row.dataset.score!=='');const score=scored.length?Math.round(rowTotal(scored,'score')/scored.length):null;summaryScore.textContent=score===null?'—':String(score);summaryScore.className=score===null?'':'rating-'+scoreRating(score,incomplete);summaryVariants.textContent=String(rowTotal(matches,'variants'));summaryFindings.textContent=String(rowTotal(matches,'findings'));}
function updateGroupSummary(group,matches){if(!matches.length)return;const scored=matches.filter(row=>row.dataset.score!=='');const score=scored.length?Math.round(rowTotal(scored,'score')/scored.length):null;const incomplete=rowTotal(matches,'incomplete');group.querySelector('.group-variants').textContent=String(rowTotal(matches,'variants'));const scoreElement=group.querySelector('.group-score');scoreElement.textContent=score===null?'—':String(score);scoreElement.className='group-score rating-'+scoreRating(score,incomplete);const findingsElement=group.querySelector('.group-findings .findings-hover');findingsElement.textContent=String(rowTotal(matches,'findings'));const findingText=matches.flatMap(row=>{const text=row.querySelector('.target-findings .findings-hover')?.dataset.tooltip;return text&&text!=='no findings'?text.split('\\n'):[];});findingsElement.dataset.tooltip=findingText.length?findingText.join('\\n'):'no findings';}
