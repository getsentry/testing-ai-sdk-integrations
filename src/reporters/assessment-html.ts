...
function variantDetail(assessment: VariantAssessment): string {
	...
	return `<div class="detail-content">
		...
		<div class="secondary-stack">${artifactDetails(assessment)}</div>
	</div>`;
}
...
function targetDetail(target: TargetAssessment): string {
	return `<div class="target-detail"><div class="variant-header"><span>variant</span><span class="variant-meta">configuration / versions</span><span class="variant-findings">findings</span><span class="variant-score">score</span><span class="variant-trend">trend</span></div><div class="variant-stack">${target.variants
		.map(
			(variant, index) =>
				`<details class="variant-panel"><summary><strong class="variant-name">variant ${index + 1}</strong><span class="variant-meta">${variantMetadata(variant)}</span><strong class="variant-findings">${findingsHover(variant.findings)}</strong><strong class="variant-score rating-${classifyScore(effectiveScore(variant), variant.completion)}">${effectiveScore(variant)}</strong><span class="variant-trend">${scoreSparkline("variant", variant.id)}</span></summary>${variantDetail(variant)}</details>`,
		)
		.join("")}</div></div>`;
}

function targetRow(target: TargetAssessment, index: number): string {
	...
	</tr><tr id="${rowId}" class="detail-row" hidden><td colspan="6">${targetDetail(target)}</td></tr>`;
}
	...
	const rows = [...targetsByFramework.entries()]
		...
		.map(([framework, targets]) => {
			...
			return `<tbody data-framework-group="${escapeHtml(framework)}" data-expanded="false"><tr class="framework-group" tabindex="0" aria-expanded="false"><th colspan="2"><div class="framework-heading"><strong>${escapeHtml(framework)}</strong><small>${targets.length} platform${targets.length === 1 ? "" : "s"}</small></div></th><th class="group-variants">${variantCount}</th><th class="group-findings">${findingsHover(groupFindings)}</th><th class="group-score rating-${groupRating}">${groupScore}</th><th class="group-trend">${scoreSparkline("framework", framework)}</th></tr>${[
				...
				.map((target) => targetRow(target, rowIndex++))
				.join("")}</tbody>`;
		});
...
const search=document.getElementById('search');const summaryScore=document.getElementById('summary-score');const summaryVariants=document.getElementById('summary-variants');const summaryFindings=document.getElementById('summary-findings');let searchActive=false;
...
function scoreColor(score){if(score>=85)return'#147d64';if(score>=70)return'#a36a0a';return'#b4532a';}
...
document.addEventListener('click',event=>{const link=event.target.closest('[data-span-target]');if(link)setTimeout(()=>revealSpan(link.dataset.spanTarget),0);});
window.addEventListener('hashchange',()=>revealSpan(location.hash.slice(1)));if(location.hash)setTimeout(()=>revealSpan(location.hash.slice(1)),0);
...
export async function writeAssessmentHtml(
	...
	await writeFile(reportPath, renderAssessmentHtml(report), "utf8");
	return reportPath;
}
