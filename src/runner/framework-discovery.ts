...
export function discoverFrameworks(): DiscoveredFramework[] {
  ...
  for (const category of categories) {
    ...
    const platforms = fs.readdirSync(categoryPath, { withFileTypes: true })
      .filter(dirent => dirent.isDirectory() && (dirent.name === 'js' || dirent.name === 'py'))
      .map(dirent => dirent.name);
    ...
    for (const platform of platforms) {
      ...
      const frameworkDirs = fs.readdirSync(platformPath, { withFileTypes: true })
        .filter(dirent => dirent.isDirectory() && !dirent.name.startsWith('.'))
        .map(dirent => dirent.name);
      ...
      for (const frameworkDir of frameworkDirs) {
        ...
        try {
          ...
          if (!config.name || !config.displayName || !config.type || !config.platform) {
            console.warn(`Warning: Invalid config in ${configPath} - missing required fields`);
            continue;
          }
          
          // Validate platform matches directory structure
          if (config.platform !== platform) {
            console.warn(`Warning: Platform mismatch in ${configPath} - expected ${platform}, got ${config.platform}`);
            continue;
          }
 ...
 */
export function getFrameworksByCategory(category: string): DiscoveredFramework[] {
 ...
 */
export function getFrameworksByPlatform(platform: 'js' | 'py'): DiscoveredFramework[] {
 ...
 */
export function getFrameworksByType(type: 'llm-only' | 'agentic'): DiscoveredFramework[] {
 ...
 */
export function getFrameworkByName(name: string): DiscoveredFramework | undefined {
...
export function listFrameworks(): void {
  ...
  // Group by category
  const byCategory = frameworks.reduce((acc, f) => {
